...
const DEFAULT_ASSISTANT_SCORING_ENABLED = false;
...
const sidebarMenuGroups = [
  {
    title: 'ТУРНИР',
    items: [
      { id: 'players', label: 'Жөндөөлөр', badge: '•' },
      { id: 'playoff', label: 'Жеке элек', badge: '' },
    ],
  },
  {
    title: 'БӨЛҮМДӨР',
    items: [
      { id: 'journal', label: 'Тандоо элек', badge: '' },
      { id: 'rating', label: 'Даража', badge: '' },
      { id: 'report', label: 'Баяндама', badge: '' },
      { id: 'playerData', label: 'Катышуучулар', badge: '' },
    ],
  },
];
...
const normalizeAssistantScoringEnabled = (value) => (typeof value === 'boolean' ? value : DEFAULT_ASSISTANT_SCORING_ENABLED);
...
const renamePlayerInBracket = (bracket, playerId, nextName) => ({
  ...
  winners: (bracket.winners || []).map((entry) =>
    entry?.player?.id === playerId ? { ...entry, player: { ...entry.player, name: nextName } } : entry,
  ),
});
...
const App = ({ onLogout }) => {
  ...
  const [passwordProtectionEnabled, setPasswordProtectionEnabled] = useState(initialState.passwordProtectionEnabled ?? DEFAULT_PASSWORD_PROTECTION_ENABLED);
  const [assistantScoringEnabled, setAssistantScoringEnabled] = useState(initialState.assistantScoringEnabled ?? DEFAULT_ASSISTANT_SCORING_ENABLED);
  ...
  const applyTournamentState = (nextState) => {
    ...
    setPasswordProtectionEnabled(nextState.passwordProtectionEnabled ?? DEFAULT_PASSWORD_PROTECTION_ENABLED);
    setAssistantScoringEnabled(nextState.assistantScoringEnabled ?? DEFAULT_ASSISTANT_SCORING_ENABLED);
  };
  ...
  const advance = (currentStage) => {
    updateCompetitionState(viewDivision, (current) => {
      const stageKeys = getVisibleStageKeys(current.playoffMode);
      const currentStageIndex = stageKeys.indexOf(currentStage);
      const sourceStage =
        (current.bracket[currentStage] || []).length > 0
          ? currentStage
          : currentStageIndex > 0 && (current.bracket[stageKeys[currentStageIndex - 1]] || []).length > 0
  ...
  const finishTournament = () => {
    ...
    setBracket((prev) => ({
      ...prev,
      winners: [
        { position: 1, player: bracket.final12.winner },
  ...
  return (
    <div className={`app-shell admin-shell ${activeTab === 'journal' || activeTab === 'report' ? 'admin-shell--sheet-view' : ''}`}>
      <div className="app-background" aria-hidden="true" />

      <header className="admin-mobile-bar" aria-label="Админ мобил меню">
        <div className="admin-mobile-bar__brand">
          <div className="admin-sidebar__vuexy-mark">
            <TargetIcon size={18} />
          </div>
          <div className="admin-mobile-bar__copy">
            <p className="eyebrow">Vuexy</p>
            <strong>Admin</strong>
          </div>
        </div>
        <button
          type="button"
          className={`menu-toggle admin-menu-toggle ${isMenuOpen ? 'menu-toggle--open' : ''}`}
          onClick={() => setIsMenuOpen((current) => !current)}
          aria-label={isMenuOpen ? 'Меню жабуу' : 'Меню ачуу'}
      ...
      <div className="admin-layout">
        <aside id="admin-sidebar" className={`admin-sidebar ${isMenuOpen ? 'admin-sidebar--open' : ''}`}>
          <div className="admin-sidebar__menu">
            ...
            {sidebarMenuGroups.map((group) => (
              <div key={group.title || 'primary'} className="admin-sidebar__group">
                {group.title && <p className="admin-sidebar__group-title">{group.title}</p>}
                <div className="admin-sidebar__group-items">
                  {group.items.map((item) => (
                    <button
                      key={`${group.title || 'primary'}-${item.id}-${item.label}`}
                      type="button"
                      onClick={() => {
                        setActiveTab(item.id);
                        setIsMenuOpen(false);
                      }}
                      className={`tab-button admin-sidebar__item ${activeTab === item.id ? 'tab-button--active admin-sidebar__item--active' : ''}`}
                    >
                      <span className="admin-sidebar__label">{item.label}</span>
                      {item.badge ? <span className={`admin-sidebar__badge ${item.badge === 'new' ? 'admin-sidebar__badge--new' : ''}`}>{item.badge}</span> : null}
            ...
            <div className="admin-sidebar__footer">
              ...
              <button type="button" className="theme-toggle" onClick={() => window.location.assign('/assistant/')}>
                Жардамчы
              </button>

              {onLogout && (
                <button type="button" className="theme-toggle auth-logout-button-inline" onClick={onLogout}>
                  Чыгуу
                </button>
              )}
            </div>
          ...
          <div className="layout-grid">
            <section className="panel">
              ...
              <div className="form-grid">
                ...
                <div className="field">
                  <span className="field__label">Жеке жардамчы</span>
                  <div className="mode-switch">
                    <button
                      type="button"
                      onClick={() => setAssistantScoringEnabled(false)}
                      className={`mode-switch__button ${!assistantScoringEnabled ? 'mode-switch__button--active' : ''}`}
                    >
                      Өчүк
                    </button>
                    <button
                      type="button"
                      onClick={() => setAssistantScoringEnabled(true)}
                      className={`mode-switch__button ${assistantScoringEnabled ? 'mode-switch__button--active' : ''}`}
                    >
                      Күйүк
                    </button>
                  ...
                  <div className="note-card">
                    {assistantScoringEnabled
                      ? 'Бул режимде оюнчулар упай жаза албайт. Упайды жардамчы админ гана киргизет.'
                      : 'Кадимки режимде оюнчулар өз упайларын жаза алышат.'}
                  </div>
                </div>
          ...
          <section className="report-page">
              ...
              <div className="panel__header panel__header--stack">
                <div>
                  ...
                  <div className="score-round-manager__summary">
                    ...
                    {assistantScoringEnabled && (
                      <p className="score-round-manager__hint score-round-manager__hint--warning">
                        Плей-офф жабык — упайларды азыр жардамчы админ гана киргизет.
                      </p>
                    )}
            ...
            >
              <div className="report-sheet__topline">
                <span>Өткөрүлгөн жери: {location}</span>
          ...
          <section className="panel">
            <div className="panel__header">
              <div>
                ...
                {assistantScoringEnabled && (
                  <p className="panel__subtitle panel__subtitle--warning">
                    Плей-офф жабык — оюнчулар упай жаза албайт, жазуу жардамчы админ аркылуу гана болот.
                  </p>
                )}
              </div>
            </div>
  ...
  if (match.isFinal) {
    return (
      <article className={`match-card match-card--final ${match.winner ? 'match-card--winner' : ''}`}>
        {match.autoRecalculated && <span className="playoff-card__badge">обновлено</span>}
        <FinalPlayer
          playerNumber={1}
